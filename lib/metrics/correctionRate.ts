...
const CORRECTION: RegExp[] = [
  ...
  /(?:そう)?じゃなくて|そうではなく/, // そうじゃなくて（前の出力を否定して言い直す形）
  // 注: 単独の「ではなく」は初出の対比依頼（例「AではなくBを作って」）で誤検出するため除外。
  /間違(?:って|い|え)|まちが(?:って|い)/, // 間違って / 間違い
  /違(?:くて|って)/, // 違くて / 違って（口語）
  /(?:そう|それ|これ)じゃない/, // そうじゃない 等
  // 注: 単独の「おかしい」は初出の不満・雑談でも出るため、出力参照を伴う形のみに限定する。
  /(?:答え|回答|出力|結果|それ|これ)(?:は|が)?.{0,4}おかしい/,
